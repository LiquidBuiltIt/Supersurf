...
- Playbooks now replay every trail-recorded tool, not just `browser_interact`: `create` accepts any id from `history`, `run` re-issues navigate/extract/tabs/etc. with their frozen params (read-tool output is included in the run result), and the start-URL auto-navigate is skipped when step 1 is itself a navigate. Healing remains selector-based (click/hover/drag).
- **Known limitation: playbook healing is verb-limited.** `create` and `run` heal element-selector drift only for `click`, `hover`, and `drag` actions. A run step using another verb (e.g. `type`, `select_option`, `file_upload`) fails without a heal attempt and reports the failure. Widen healing to all selector-resolving verbs is a separate chore (deferred), tracked in `BACKLOG.md` #2.

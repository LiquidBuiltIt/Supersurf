...
- feat: `supersurf profiles create <name>` and `supersurf profiles rm <name>` (alias `delete`) manage managed Chromium profiles from the terminal — both route through the daemon's existing `profiles.create` / `profiles.delete` IPC (same failsafes as the MCP tools: `rm` refuses while active sessions are connected or a user-opened browser is running for that profile).

...
- feat: extension Settings “Keep browser after session ends” (**opt-in**, default off) — when enabled, daemon skips SIGTERM on last MCP session disconnect; idle/shutdown/orphan kills unchanged
- feat(extension): persist Chrome Tab Groups by `client_id` across MCP disconnect — idle groups stay titled and turn grey; reconnect reclaim restores the prior active color from `chrome.storage.local`
- **feat: `screenshot.omit_path` in `~/.supersurf/config.json`** (`inline` \| `path` \| `both`, default **`inline`**) — controls `browser_take_screenshot` when `path` is omitted. `path` saves under `$TMPDIR/supersurf-screenshots/` (text only); `both` saves and returns an inline image. Explicit `path` still wins. Env: `SUPERSURF_SCREENSHOT_OMIT_PATH`. Internal `rawResult` captures stay inline.

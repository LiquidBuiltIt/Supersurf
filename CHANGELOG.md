...
- **change(playbooks): `name` and `purpose` are now marked required in the `browser_interact` schema for element-targeting actions that carry a selector** (click/type/clear/hover/select_option/select_custom/file_upload). This is advisory only — this codebase does not validate MCP tool inputs server-side, so a call omitting them still executes; the schema change is a hint to the calling agent, not enforcement. It reverses the earlier never-reject stance in intent: without a hard requirement agents named elements inconsistently, and a corpus full of unnamed records makes resolve-by-name unusable. The requirement is conditional (JSON Schema `allOf`/`if`/`then`) so coordinate-only clicks, `press_key`, `wait`, the scroll actions and `force_pseudo_state` are unaffected. It is deliberately **not** gated on the `fingerprinting` experiment — `tools/list` is served before `connect` populates the experiment state and the server emits no `tools/list_changed`, so a gated requirement would always read "off". A malformed name is still normalized server-side, never rejected.

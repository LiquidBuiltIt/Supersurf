...
- **feat: fingerprint healing now covers every selector-resolving `browser_interact` verb** — `click`, `hover`, `drag`, `type`, `clear`, `select_option`, `select_custom`, `scroll_to`, `scroll_by`, `scroll_into_view`, `wait`, and `file_upload`, plus `browser_fill_form` fields. `force_pseudo_state` is the sole exception (drives CDP directly off a raw `objectId`, nothing to heal against).

...
- **change(playbooks): `name` and `purpose` are now required on `browser_interact` for element-targeting actions that carry a selector** (click/type/clear/hover/select_option/select_custom/file_upload). This reverses the earlier never-reject stance: without a hard requirement agents named elements inconsistently, and a corpus full of unnamed records makes resolve-by-name unusable. The requirement is conditional (JSON Schema `allOf`/`if`/`then`) so coordinate-only clicks, `press_key`, `wait`, the scroll actions and `force_pseudo_state` are unaffected. It is deliberately **not** gated on the `fingerprinting` experiment — `tools/list` is served before `connect` populates the experiment state and the server emits no `tools/list_changed`, so a gated requirement would always read "off". Presence only: a malformed name is still normalized server-side, never rejected.
- **feat(playbooks): `browser_snapshot` and `browser_lookup` now show the handle you recorded for an element instead of just its selector.** Any element previously named through `browser_interact` renders as `first_name_input [input#fname]` — handle first, the CSS kept alongside so there is always a working fallback. Elements with no recorded handle render exactly as before, byte-identical. This closes the discovery gap the resolve-by-name round left open: an agent could previously only use a handle it had named itself in the same session. Scoped to output that actually carries a selector — `browser_lookup` matches and `browser_snapshot`'s Form Fields section; the accessibility-tree lines are untouched because CDP AX nodes carry no selector to match on. One corpus read per tool call, never one per node. Gated behind `fingerprinting`; no-op when off.
- **perf(fingerprinting): the fingerprint corpus is no longer re-read and re-parsed on every lookup.** `loadDomain` previously did a `readFileSync` + full `JSON.parse` per call, and `getRecord` called it again internally — so resolving a handle cost two complete parses of the domain file where a plain selector cost one. It now memoizes the parsed store, guarded by the file's mtime **and** size so a write from another server process (one per MCP client, all sharing the corpus) is picked up on the next read rather than served stale.

...
- **feat: one command installs SuperSurf.** `curl -fsSL https://liquidbuiltit.github.io/Supersurf/install.sh | sh` downloads the compiled `supersurf` binary for the host's OS and architecture from the GitHub release assets, installs it to `~/.local/bin` without sudo, and adds that directory to the shell profile only when it is genuinely missing from `$PATH` — re-running the script is the upgrade path, and an unconditional append would accumulate a duplicate line every time. Interactive is the default: it starts the daemon, opens the Chrome Web Store listing with the OS-native opener, and then *waits for the extension's WebSocket handshake* rather than asking the user to confirm they installed it, because a press-Enter-when-done prompt reports a success it never verified. Enter skips the wait, and a timeout prints `supersurf daemon status` as the named next step instead of going quiet. Mode selection reads `/dev/tty`, not stdin — piping the script into `sh` makes stdin the script's own text, so a bare `read` eats the lines it has not run yet; `--yes`, or no `/dev/tty` at all (CI, Docker, an agent), takes the non-interactive path, which installs the binary, prints the extension URL and exits 0. Node is checked before anything is downloaded, since the binary is self-contained but shells out to `npx` for the server and the daemon. The script never edits an MCP client config: it prints the `claude mcp add` command and lets you run it.
- `feat`: `browser_extract_content` in `mode: 'selector'` now returns every element the selector matches, separated by a `---` rule, with the count in `matches`. It previously read only the first match, so a class shared by siblings (e.g. `.WorkflowJob`) silently reported one element out of many. `max_lines` still bounds the whole result.
- **chore: the `supersurf` CLI is its own package, compiled to a standalone binary.** The entrypoint layer moved from `server/src/bin/` to a new `cli/` workspace package that Bun cross-compiles to four targets (`linux-x64`, `linux-arm64`, `darwin-x64`, `darwin-arm64`) from a single host. `cli/` is private and is never published to npm, so the `supersurf-mcp` tarball no longer ships the CLI that launches it.
- **chore: `supersurf-mcp` declares exactly one bin, so a global install no longer puts `supersurf` on your PATH.** The bin map is now `supersurf-mcp` → `dist/cli.js` alone; the `supersurf` and `supersurf-daemon` bins the package also declared are gone. `npx supersurf-mcp@<version>` is unchanged and every MCP client config keeps working untouched. What changes is `npm i -g supersurf-mcp`: it used to put `supersurf`, `supersurf-mcp` and `supersurf-daemon` on PATH and now installs `supersurf-mcp` only. If a `supersurf …` or `supersurf-daemon …` command stopped resolving after an upgrade, that is why — the daemon is reachable as `npx supersurf-daemon@latest <cmd>`, and `supersurf-daemon` remains the daemon package's own bin.
- **chore: the compiled binary reaches the npm packages through a version-pinned `npx`.** A compiled binary carries no `node_modules`, so `supersurf mcp` runs `npx supersurf-mcp@<the binary's own version>` and `supersurf daemon <cmd>` runs `npx supersurf-daemon@<the same version>`, both with `stdio: 'inherit'` so the child gets the real file descriptors — `mcp` is a stdio protocol server and a relay would corrupt its JSON-RPC stream. The pin is always the binary's own version and never `@latest`, so a CLI can never launch a server speaking a protocol it does not know.
- **chore: `playbook ls`, `inspect`, `validate` and `migrate` run entirely inside the binary; only `playbook run` shells out.** The read-only renderers moved from `server/src/tools/playbooks.ts` to a dependency-light `server/src/playbooks/report.ts`, so listing or validating a script no longer boots the ConnectionManager and needs no npx fetch at all. `run` still shells out because it alone reaches the screenshot tool's native `sharp` dependency.
- fix: **`supersurf-mcp` understands `playbook run`, so a terminal playbook run has somewhere to land.** The `supersurf` binary shells out to `npx supersurf-mcp@<version> playbook run <name> --param k=v` — it cannot run a playbook itself, because `playbooks/runner.ts` pulls the ConnectionManager and, through `tools/screenshot.ts`, the `sharp` native addon. But the receiving program was option-only, so that argv died on `error: unknown option '--param'` and, without flags, on `error: too many arguments. Expected 0 arguments but got 3.` `supersurf-mcp` now registers a `playbook run <name>` subcommand taking `--param` (repeatable), `--profile` and `--json`, which re-parses its own arguments against the script's declared `meta.params` rather than trusting the caller's pre-flight, and renders the same success/failure output — including the page snapshot captured before the run's tab is closed. `run` is the only playbook subcommand on this side; `ls`, `inspect`, `validate` and `migrate` stay in the binary. A bare `npx supersurf-mcp` still starts the MCP server and still accepts the legacy leading `mcp` word, unchanged. The terminal path continues to ignore `security.playbook_eval` — that gate exists because an agent is an untrusted caller, not the human who can read the file first.
- fix: **`npx supersurf-mcp@latest` starts the MCP server again, and no bin rewrites the argv it was handed.** `supersurf-mcp` spliced `'mcp'` into `process.argv` before delegating to the bin dispatcher, so the command printed in the README and in the CLI's own help text — `npx supersurf-mcp@latest mcp` — reached Commander with a duplicate positional and died on `error: too many arguments. Expected 0 arguments but got 1.` The bin now imports the server CLI directly. The legacy trailing `mcp` word is still accepted and silently ignored, so an MCP client config written from the old README keeps working untouched. `supersurf-daemon` spliced `'daemon'` identically; it never crashed only because the daemon's arg parser ignores unknown tokens, and it now passes argv straight through to the resolved daemon entry. Both bins also drop a deprecation notice that told users to run `supersurf mcp` / `supersurf daemon` — the bare `supersurf` name on npm is a permanently-squatted placeholder that will never be ours. Separately, the `playbooks` fingerprinting gate told the agent to run `npx supersurf daemon restart`, which fetches that stranger's package; it and the two unpinned `npx supersurf-daemon status` hints now read `npx supersurf-daemon@latest <cmd>` like every other daemon hint. `supersurf` keeps the dispatcher and owns routing only.
- feat: **the daemon now checks the extension's version at the handshake and refuses one it cannot work with.** A stale extension left installed after a server upgrade used to complete the handshake, get matched to a profile, and then fail in whatever way its missing features happened to fail — with nothing naming the real cause. A major or minor mismatch is now rejected and the socket closed with code `4001`; a patch difference is logged and allowed. The rejection reaches the caller rather than only the log: the pending matchmaker request fails and in-flight requests drain with the version error as their reason, so `profiles.connect` returns a named cause instead of a timeout, and `connect` reports the version error in place of the generic "not connected" message — and keeps reporting it, because a later `status` used to render a bare "Disabled" with no trace of the reason. A rejection is scoped to the connection slot it came from, so a rejected managed profile never poisons the unmanaged slot or the reverse, and it never outranks a connection that works: the matchmaker checks the pool before the rejection map, so a second browser running a stale extension cannot lock out the healthy one already serving the slot they share — most easily the unmanaged slot, which every bring-your-own Chromium shares. If either version is missing or unparsable the guard cannot run, so it fails open and says so on stderr — a guard that quietly stops guarding is worse than no guard.
- fix: **a failed extension download now blocks the profile spawn that needs it, instead of launching a browser without one.** If the daemon could not pull the extension and nothing is cached, `spawnProfile` throws a message naming the profile, the underlying cause, and `npx supersurf daemon restart`. The guard is conjoined with the cache check, so a later successful pull makes it a no-op and it can never refuse a working setup.
- fix: **the profile registration page no longer claims a success it cannot prove.** It used to add its success state one line after `window.postMessage`, so it reported "registered" even when the extension never received the message — and the extension never answered, because no background listener replied to the registration message at all. The background script now responds once the profile is actually written to `chrome.storage.local`, the content script acknowledges only on that reply, and the page waits for the acknowledgement and shows a visible failure state with recovery steps if it does not arrive. The profile name is now escaped for both the HTML and the inline-script contexts it is interpolated into. The content script's half of that round trip is written inline and exports nothing: an MV3 content script is a classic script, so a single `import` or `export` statement makes Chrome refuse to parse the whole file and silently kills profile registration, console capture and tech-stack detection together. `extension/build.ts` now fails the build if a module statement reaches `dist/content-script.js`, and `npm run smoke.register` drives the real page through a real headless Chromium to prove the ack round trip binds the profile.
- security: **the AST layer now catches a prototype walk reached by destructuring or a template-literal key.** The walker visited six node types and `ObjectPattern` was not one of them, so `const { constructor: C } = supersurf` and `` supersurf.click[`constructor`] `` passed validation while the plain `supersurf.click.constructor` was blocked. Destructured keys are now checked against the same MemberExpression rules member access gets, a computed key that is a template literal with no interpolation is normalized to its string form first, and `__lookupGetter__` / `__lookupSetter__` join `__proto__` / `constructor` on the prototype-walk rule. Ordinary destructuring of non-blocked names is unaffected. The same walker backs `browser_evaluate`'s `secure_eval`, which gets the identical tightening.
- security: **the playbook sandbox re-hashes a script immediately before it runs.** `refreshRegistry()` gates its read on mtime+size, so a same-size rewrite at an unchanged explicit mtime never reaches the sha256 comparison and the cached `ValidationRecord` keeps reporting `valid: true` for bytes nobody analyzed. The registry is the listing path; the execution path must not inherit its shortcut. `runPlaybookScript` (`security/sandbox/host.ts`) now hashes the source it is about to send to the child and refuses unless it equals the hash the record was validated against, so what was analyzed and what executes are provably the same bytes.
- security: **an unbuilt checkout no longer runs playbook code with the process cage off.** `resolveChildEntry` falls back to `child.ts` via `tsx` when no compiled `child.js` exists, and `permissionFlagsFor` returns no flags for a `.ts` entry because the tsx loader needs access the Node permission model would deny — so the untrusted child spawned with unrestricted filesystem read. `runPlaybookScript` now refuses that fallback and points at `npm run build`. Setting `SUPERSURF_ALLOW_UNSANDBOXED_PLAYBOOK=1` proceeds anyway but logs a warning naming exactly what protection is off. A published install and any built checkout never reach this path.
- chore: retired-but-kept code (`*.old.ts`, `*.old/`) is now excluded from every package build, and its stale compiled output is deleted. `secure-eval.old` was still being compiled into `server/dist/` and `extension/dist/`, so dead code shipped to users in both published packages and in the CWS bundle. This also deletes `playbooks/store.js` and `playbooks/format.js` — orphaned `dist/` artifacts whose sources went away with the JSON playbook format.
- fix: a playbook run now checks the result of opening its own tab instead of discarding it. `browser_tabs` `new` doesn't throw on failure — it returns `{ success: false, error }` as data — so a failed open used to fall through unnoticed, drive the whole script over whatever tab the calling agent had attached, and CLOSE it at teardown. The run now fails immediately with the open's own error instead, and a throw from that call is folded into the same failure path so it can no longer escape teardown and leak the run's daemon session (BACKLOG #23).
- fix: **the playbook sandbox now runs the COMPILED child, so it starts in a clean install.** `resolveChildEntry` looked for `child.js` only next to itself, so a source checkout always fell through to `child.ts` via `tsx` — a devDependency resolved through an npx cache — and a published install that reached the fallback had no `tsx` at all. It now prefers `dist/security/sandbox/child.js` (the published copy, or the built mirror of a `src/` checkout) and spawns it under plain `node` with no TypeScript loader in the process that runs untrusted playbook code; `tsx` + `child.ts` is the fallback for an unbuilt checkout only, and both failure paths now name every file they looked for. The child's `--allow-fs-read` scope also widens from the package root to the hoisted `node_modules` above it: npm hoists `acorn` out of `supersurf-mcp/`, so the compiled child died with `ERR_ACCESS_DENIED` before its first frame. Write access is still never granted.
- security: **the playbook vm no longer hands a playbook a host-realm value.** `codeGeneration: { strings: false, wasm: false }` is scoped to the vm realm only, so an injected host function kept its host-realm `.constructor` and `supersurf.click.constructor('return this')()` returned the host global in one line. `createPlaybookContext` now hands the host values over on one temporary key and runs a host-compiled bootstrap that rebuilds vm-realm equivalents onto `globalThis` — functions wrapped in vm async arrows, resolved values JSON round-tripped through the vm's `JSON`, envelope failures rethrown as vm `Error`s, `params` rebuilt at every depth — closing the function, promise, resolved-value and thrown-error ladders. The sandbox child calls the playbook's default export with the re-realmed values off the context instead of its own host locals, so the destructured `{ supersurf, params, log }` argument is closed too. Permission-by-construction is unchanged: a method `buildClient` declined to create is not resurrected.
- feat!: playbooks are JavaScript files. A `~/.supersurf/playbooks/<name>.playbook.js` exports `meta` (description, params, profile, startingPoint, permissions) and a default async function taking `{ supersurf, params }`. A JSON playbook was a recorded value; a script takes arguments, branches and returns data — parameterization is the whole gap.
- feat!: the `playbooks` tool actions are now `list | inspect | validate | run | history`. `create` is **deleted with no replacement**: SuperSurf never writes a playbook file. The agent authors the script with its own file tools, which is why `history` survives untouched — it reads the in-memory action trail and hands back the selectors a working run actually used.
- feat!: `supersurf playbook` is `ls | inspect | validate | run | migrate`. `edit`, `rm`, `export` and `import` are deleted — a `.playbook.js` file is edited with an editor, removed with `rm` and copied with `cp`.
- feat: `supersurf playbook migrate` converts legacy JSON playbooks to `.playbook.js` one-shot. `purpose` → `meta.description`, `profile` → `meta.profile`, the first navigate step's URL host → `meta.startingPoint`. A migrated script has **no params** — a recording has none. Unmappable steps become `// TODO` lines carrying the original rather than a guess. The JSON reader is deleted in this same release, so run it before upgrading past this version.
- feat: `meta.experiments: true` opts a single run into every available experiment for that run's session only. It never writes `~/.supersurf/config.json` and never affects the calling agent's session, and the run record carries an `experiments` flag so a failure can be attributed to the experimental path.
- feat: `security.playbook_eval` config leaf (default `true`, i.e. allowed; `SUPERSURF_DISABLE_PLAYBOOK_EVAL=1`, `--disable-playbook-eval`). Caller-based: enforced when an agent invokes `playbooks run`, ignored by `supersurf playbook run` at a terminal.
- feat: every run appends a record to `~/.supersurf/playbooks/<name>.runs.jsonl` — ok/error, duration, params, and on failure a page snapshot in `evidence`. A run owns its tab and closes it at the end, so without the capture every failure would read as "it broke, no idea why".
- feat: a playbook run gets its own daemon session and its own tab, closed when the run ends. It no longer borrows the caller's attached tab, so a run cannot leave the agent's tab somewhere unexpected.
- feat: validation is stat-on-tool-call — the verdict for every script rides the next tool result, keyed by content hash so an unchanged file is never re-parsed.
- feat: the playbook discovery hint now keys off `meta.startingPoint` instead of domains derived from recorded step URLs. A script has no recorded URLs, so the author declares the entry point.
- chore: `server/src/backend-config.ts` (`buildConfigService()` + `backendConfigFrom()`) is shared by `server/src/cli.ts`, `server/src/bin/playbook-cli.ts` and the playbook runner instead of being copy-pasted per entry point.
...
- fix: `stripStatusHeader` no longer drops a `_recovery` note (tab re-bound mid-call) along with the status header — the note now survives into playbook step output.
- fix: `playbooks action='run'` now attaches a dialog notice under the specific step that raised it instead of silently dropping the notice (replayed steps bypass the live-call dispatcher, which is where dialog draining otherwise happens).
- fix: `playbooks action='run'` now includes the inline image block when a replayed `browser_take_screenshot` step was originally captured without a `path` — previously the blob was dropped from the run result and only the text summary came through. Path-recorded screenshots are unaffected.

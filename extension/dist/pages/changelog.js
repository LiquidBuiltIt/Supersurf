 ...
 * has nothing to show.
 */
import { sliceSections, renderBulletHtml } from './changelog-render.js';
...
function renderSections(data, from, curr) {
    const sectionsEl = document.getElementById('sections');
    if (!sectionsEl)
        return;
...
async function init() {
    ...
    if (fromEl)
        fromEl.textContent = from ?? 'a previous version';
    try {
        ...
        if (!data || !Array.isArray(data.sections))
            throw new Error('malformed changelog.json');

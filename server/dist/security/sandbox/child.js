...
async function start(init) {
    ...
    const log = (message) => emit({ t: 'log', message: String(message) });
    const supersurf = (0, client_1.buildClient)(send, Array.isArray(meta.permissions) ? meta.permissions : []);
    try {
        ...
        const context = (0, context_1.createPlaybookContext)({ supersurf, params, log });
        (0, context_1.compilePlaybook)(String(init.source ?? ''), String(init.file ?? 'playbook.js')).runInContext(context);
        const entry = context[context_1.DEFAULT_EXPORT_KEY];

 ...
 * click path.
 */
function armProbeExpr(targetExpr) {
    return `(() => {
    try {
      const t = ${targetExpr};
      const w = window;
 ...
 */
function describeClickOutcome(target, probe, hadSpawn) {
    ...
    if (!probe || probe.ok !== true || !probe.hadTarget)
        return base;
...
(0, registry_1.registerAction)({
    ...
    async run(ctx, action) {
      ...
      if (el && (el.closest('a[href]') || el.onclick)) el.click();
        ...
        const probe = await (0, frames_1.evalInFrameOrTop)(ctx, READ_PROBE_EXPR, clickContextId).catch(() => null);

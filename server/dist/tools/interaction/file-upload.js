...
(0, registry_1.registerAction)({
    ...
    `;
        const resolve = async () => {
            ...
            if (!objectId) {
                const match = await (0, frames_1.findElementInFrames)(ctx, resolveExpr);
                if (match) {
            ...
            if (!objectId) {
                ...
                const present = await ctx.eval(`[...document.querySelectorAll('input[type="file"]')].slice(0, 5).map(i =>
            i.id ? '#' + i.id : (i.name ? 'input[name="' + i.name + '"]' : 'input[type=file]'))`).catch(() => []);
        ...
        try {
            ...
            if (/object id|could not find object/i.test(msg)) {
                ...
                throw new Error(`${action.selector} resolved to an element that is not a file input and contains ` +

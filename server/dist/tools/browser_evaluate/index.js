    ...
    if (code && !process.env.SUPERSURF_DISABLE_SECURE_EVAL) {
        ...
        if (!analysis.safe) {
            return ctx.error(`Code blocked by \`secure_eval\`.\n\n` +
                ...
                `If you genuinely need this primitive, set \`SUPERSURF_DISABLE_SECURE_EVAL=1\` in the server env.`, options);
        }
        ...
        try {
            ...
            if (validation && validation.safe === false) {
                return ctx.error(`Code blocked by \`secure_eval\` (membrane).\n\n` +
                    ...
                    `or set \`SUPERSURF_DISABLE_SECURE_EVAL=1\` in the server env.`, options);
            }
        }
        catch {

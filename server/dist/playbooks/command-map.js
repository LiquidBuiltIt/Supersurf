...
function fillFields(fields) {
    if (Array.isArray(fields) || fields === null || typeof fields !== 'object')
        return fields;
    return Object.entries(fields).map(([selector, value]) => ({ selector, value }));
}
...
exports.KNOWN_METHODS = Object.keys(MAP).sort();
/**
 ...
 */
function flattenOpts(p) {
    if (!p || typeof p !== 'object' || p.opts === undefined)
        return p ?? {};
    const { opts, ...rest } = p;

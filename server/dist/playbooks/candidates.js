...
exports.selectorTokens = selectorTokens;
exports.candidateExpression = candidateExpression;
exports.captureCandidates = captureCandidates;
...
function selectorTokens(selector) {
    ...
    for (const word of words) {
        ...
        for (const part of word.split(/[-_]+|(?<=[a-z0-9])(?=[A-Z])/)) {
            ...
            if (token.length < MIN_TOKEN || STOP.has(token) || seen.has(token))
                continue;
            seen.add(token);
            out.push(token);
        }
 ...
 */
function candidateExpression(tokens, limit) {
    return `() => {
...
async function captureCandidates(backend, selector) {
    try {
        ...
        const candidates = raw
            ...
            .map((c) => {
            const out = { selector: String(c?.selector ?? '') };

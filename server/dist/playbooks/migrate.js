...
exports.stepToSource = stepToSource;
...
function stepToSource(step) {
    ...
    if (step.tool === 'browser_interact') {
        ...
        if (call)
            return { line: `  await ${call}`, manual: false };
    }
    if (step.tool === 'browser_navigate') {
        if (p.action === 'url' && typeof p.url === 'string')
            return { line: `  await supersurf.goto(${q(p.url)});`, manual: false };
        if (p.action === 'back')
            return { line: '  await supersurf.back();', manual: false };
        if (p.action === 'forward')
            return { line: '  await supersurf.forward();', manual: false };
        if (p.action === 'reload')
            return { line: '  await supersurf.reload();', manual: false };
    }
...
function startingPointOf(pb) {
    const nav = pb.steps.find(s => s.tool === 'browser_navigate' && typeof s.params?.url === 'string');
    const raw = nav?.params.url ?? pb.steps.find(s => typeof s.url === 'string')?.url;
    if (!raw)
        return null;
    try {
...
function toScript(pb) {
    const metaLines = [`  description: ${q(pb.purpose || pb.name)},`];
    if (pb.profile)
        metaLines.push(`  profile: ${q(pb.profile)},`);
    const sp = startingPointOf(pb);
    if (sp)
        metaLines.push(`  startingPoint: ${q(sp)},`);
...
async function runMigrate(flags = {}, opts = {}) {
    const log = opts.log ?? console.log;
    const dir = (0, paths_1.getPlaybooksDir)();
    let files;
    try {
    ...
    for (const file of files) {
        ...
        try {
            ...
            if (!pb || !Array.isArray(pb.steps))
                throw new Error('not a playbook (no steps array)');
        }
        catch (err) {
        ...
        const name = (0, paths_1.normalizeName)(pb.name || file.replace(/\.json$/, ''));
        const dest = (0, paths_1.playbookFile)(name);

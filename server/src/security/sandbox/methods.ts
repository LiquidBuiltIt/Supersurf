...
export const PERMISSION_GATED: Record<string, string> = {
  evaluate: 'eval',
};
 ...
 */
export function buildParams(spec: MethodSpec, args: unknown[]): Record<string, unknown> {
  const out: Record<string, unknown> = {};
  for (let i = 0; i < spec.params.length; i++) {
    if (args[i] !== undefined) out[spec.params[i]] = args[i];
  }
  return out;
}

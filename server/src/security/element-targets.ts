...
function resolveTarget(
  ...
  ambiguousConstNames: Set<string>
): TargetResolution {
  switch (node.type) {
    case 'Literal':
    ...
    case 'Identifier': {
      if (ambiguousConstNames.has(node.name)) {
        return {
          formError:
            `is the identifier \`${node.name}\`, which is \`const\`-bound to a string literal more than ` +
...
export function validateElementTargets(source: string, meta: PlaybookMeta): { error?: string } {
  ...
  const rawConstStrings = new Map<string, string[]>();
  walk.simple(ast, {
  ...
  const ambiguousConstNames = new Set<string>();
  for (const [name, values] of rawConstStrings) {
    if (values.length > 1) ambiguousConstNames.add(name);
    else constStrings.set(name, values[0]);
  }

  /** Checks 2-4 for one element-target argument. Returns an error message, or null. */
  function checkTarget(method: string, argNode: any): string | null {
  ...
  walk.simple(ast, {
    CallExpression(node: any) {
      ...
      if (method === 'wait') {
        ...
        if (!arg) return;
        if (arg.type === 'Literal' && typeof arg.value === 'number') return; // a numeric wait is a delay, not a target
